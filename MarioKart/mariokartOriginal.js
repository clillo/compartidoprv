...
function MarioKart() {
    ...
    function enviaPos(id, x, y){
        ...
        const url = 'eventos?id='+id+'&x='+x+"&y="+y;
        Http.open("GET", url);
        Http.send();
    }
    ...
    var oHills;
    var oTrees;
    var aCharacters = ["mario", "luigi", "peach", "a4"];
    var aPlayers = [];
    var oPlayer;
    var strPlayer = "";
    var playerCount = 3;
    ...
    function startGame() {
        ...
        var fncCount = function() {
                ...
                if (iCntStep < 4) {
                    ...
                    const Http = new XMLHttpRequest();
                    const url = 'eventos?id='+idVentana;
                    Http.open("GET", url);
                    Http.send();
                }
            }
    ...
    function startMusic() {
        ...
        oMusicEmbed.src = "media/" + strMap + ".mp3";
	...
	this.setIdVentana = function(iValue) {
		idVentana = iValue;
		console.log("Inicio de la libreria  id: "+iValue);
		var eventSource = new EventSource("/hello?id="+iValue);
		eventSource.onopen = function(e){
			console.log("Comienzo a escuchar las peticiones desde el servidor");
		};
		...
		eventSource.onmessage = function(e){
			...
			if (obj.accion=='avanzar'){oPlayer.speedinc = 1;} // up press
			else
			if (obj.accion=='detener'){oPlayer.speedinc = 0;} // up release
			else
			if (obj.accion=='izquerda'){oPlayer.rotincdir = 1;}
			else
				if (obj.accion=='izquerdaOff'){oPlayer.rotincdir = 0;}
				else
					if (obj.accion=='derecha'){oPlayer.rotincdir = -1;}
			else
				if (obj.accion=='derechaOff'){oPlayer.rotincdir = 0;}
				else
					if (obj.accion=='frenar'){oPlayer.speedinc -= 0.2;}
					else
						console.log('Accion invalida: '+obj.accion);
			 _self.emit("playerMove", oPlayer);
		}
		
		eventSource.onerror = function(e){
			console.log("Error de la libreria");
		};
	};
    ...
    function move(oKart, id) {
        ...
        enviaPos(oKart.id, Math.round(oKart.x), Math.round(oKart.y));
        
        	//console.log(oKart.x+','+oKart.y);
        ...
        if (oKart.marca1 && oKart.y>=245 && oKart.y<=260 && oKart.x>=410 && oKart.x<=500){
        	...
        	if (!oKart.marca2){
        		...
        		var todosTerminaron = 1;
        		for (var i = 0; i < aKarts.length; i++) 
        	       if (aKarts[i].vueltas<3)
        	          	todosTerminaron = 0;
	...
	oScr = document.createElement("div");
	       var oPImg = document.createElement("img");
            oPImg.src = "media/select_" + aCharacters[0] + ".png";
